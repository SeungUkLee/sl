...
interpret :: (SLangParser m, SLangTypeInfer m, SLangEval m) => FilePath -> T.Text -> m Result
interpret file code  = do
  (_, typ, val) <- interpret_ file code
...
typeinfer :: (SLangParser m, SLangTypeInfer m) => FilePath -> T.Text -> m Result
typeinfer file code = do
  (expr, typ) <- typeinfer_ file code
...
interpretWithFile file = do
  ...
  (_, typ, val) <- interpret_ striped code
...
typeinfer_ :: (SLangParser m, SLangTypeInfer m) => FilePath -> T.Text -> m (Expr, Type)
typeinfer_ file code = do
  expr <- parsing_ file code
  typ <- infer expr

  return (expr, typ)

interpret_ :: (SLangParser m, SLangTypeInfer m, SLangEval m) => FilePath -> T.Text -> m (Expr, Type, Value)
interpret_ file code = do
  (expr, typ) <- typeinfer_ file code
  val <- evaluate expr
